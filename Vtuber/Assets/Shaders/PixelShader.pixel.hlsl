...
float4 main(PS_INPUT input) : SV_TARGET
{
	...
	{
		const float3 vToL = PointLights[i].pos - input.cameraPos;
		const float distToL = length(vToL);
		const float3 dirToL = vToL / distToL;
	
		const float att = 1 / (PointLights[i].attConst + (PointLights[i].attLin * distToL) + (PointLights[i].attQuad * distToL * distToL));
	
		const float3 diffuse = PointLights[i].color * PointLights[i].diffuseIntensity * att * max(0.0f, dot(dirToL, input.cn));

		const float3 w = input.cn * dot(vToL, input.cn);
		const float3 r = (w * 2.0f) - vToL;

		const float3 specular = att * (PointLights[i].color * PointLights[i].diffuseIntensity) * spec * pow(max(0.0f, dot(normalize(-r), normalize(input.cameraPos))), 30.0f);
